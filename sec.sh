#! /bin/sh
#Usage: NBCloud proxy preinstallation
#TG:https://t.me/Red_Shore
...
initFirewall(){
    ...
    echo "Disable selinux->OK and firewalld->OK"
    sleep 1
}
...
initSsh(){
    ...
    sed -i 's%#Port 22%Port 56010%' /etc/ssh/sshd_config
    sed -i 's%#PermitRootLogin yes%PermitRootLogin yes%' /etc/ssh/sshd_config
    sed -i 's%#PermitEmptyPasswords no%PermitEmptyPasswords no%' /etc/ssh/sshd_config
    sed -i 's%#UseDNS yes%UseDNS no%' /etc/ssh/sshd_config
    egrep "UseDNS|56010|^PermitRootLogin|^PermitEmptyPasswords" /etc/ssh/sshd_config
...
AStr="Disable iptables selinux and Firewalld & Enable Aliyun Repository"
BStr="Set sshConfig and change port 22->56010"
